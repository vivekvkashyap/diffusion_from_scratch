...
void forward_timed(nn, batch_input, time, output, conv_out1, conv_outbr1, conv_outbr_max1, 
                conv_out2, conv_outbr2, conv_outbr_max2, conv_out3, conv_outbr3, conv_out4, 
                conv_outbc4, conv_outbr4, conv_out5, conv_outbc5, conv_outbr5, 
                mean_1, std_1, mean_2, std_2, mean_3, std_3, mean_4, std_4, mean_5, std_5){

    // enc block1
    conv2d_forward(inputs, conv_out1, nn->filter_1, 1, 1, 16, 28, 28);
    ...
    bias_forward(matmul_out2, nn->bias_m2, BATCH_SIZE, TIME_HIDDEN_SIZE);

    // concate output + timeembeddings
...
void backward_timed(Network *nn, batch_input, time, output, conv_out1, conv_outbr1, conv_outbr_max1, 
                conv_out2, conv_outbr2, conv_outbr_max2, conv_out3, conv_outbr3, conv_out4, 
                conv_outbc4, conv_outbr4, conv_out5, conv_outbc5, conv_outbr5, 
                mean_1, std_1, mean_2, std_2, mean_3, std_3, mean_4, std_4, mean_5, std_5){
        
    zero_grad(nn->filter_1, );
    zero_grad(nn->filter_2, );

    float *grad_output_final = malloc(sizeof(output)); 
    mse_loss_backward(output, true_noise, grad_output_final, 1, HEIGHT, WIDTH);
    
    // output layer
    float *grad_conv_outbr5 = malloc(sizeof(conv_outbr5));
    bias_conv_backward(grad_output_final, nn->grad_bias6, 1, 28, 28);
    conv2d_backward(conv_outbr5, output, nn->filter_6, grad_conv_outbr5, grad_output_final, nn->grad_filter_6, 1, 16, 1, 28, 28);

    // dec block2
    float *grad_conv_outbc5 = malloc(sizeof(conv_outbc5));
    relu_backward(conv_outbc5, grad_conv_outbr5, grad_conv_outbc5, 16, 28, 28);
    float *grad_mean_5 = malloc(sizeof(mean_5));
    float *grad_std_5 = malloc(sizeof(std_5));
    groupnorm_backward(conv_outbc5, mean_5, std_5, grad_conv_outbc5, grad_conv_outbc5, grad_mean_5, grad_std_5, 16, 4, 28, 28);
    bias_conv_backward(grad_conv_outbc5, nn->grad_bias51, 16, 28, 28);
    float *grad_conv_out5 = malloc(sizeof(conv_out5));
    conv2d_backward(conv_out5, conv_outbc5, nn->filter_51, grad_conv_out5, grad_conv_outbc5, nn->grad_filter_51, 1, 16, 16, 28, 28);
    bias_conv_backward(grad_conv_out5, nn->grad_bias5, 16, 28, 28);
    float *grad_conv_outbr4 = malloc(sizeof(conv_outbr5));
    ...
    // dec block3
    float *grad_conv_outbc4 = malloc(sizeof(conv_outbc4));
    relu_backward(conv_outbc4, grad_conv_outbr4, grad_conv_outbc4, 32, 14, 14);
    float *grad_mean_4 = malloc(sizeof(mean_4));
    float *grad_std_4 = malloc(sizeof(std_4));
    groupnorm_backward(conv_outbc4, mean_4, std_4, grad_conv_outbc4, grad_conv_outbc4, grad_mean_4, grad_std_4, 32, 4, 14, 14);
    bias_conv_backward(grad_conv_outbc4, nn->grad_bias41, 32, 14, 14);
    float *grad_conv_out4 = malloc(sizeof(conv_out4));
    conv2d_backward(conv_out4, conv_outbc4, nn->filter_41, grad_conv_out4, grad_conv_outbc4, nn->grad_filter_41, 1, 32, 32, 14, 14);
    bias_conv_backward(grad_conv_out4, nn->grad_bias4, 32, 14, 14);
    float *grad_conv_out_concat = malloc(sizeof(conv_out_concat));
    ...
    // concate output + timeembeddings
    float *grad_conv_outbr3 = malloc(sizeof(conv_out_br3));
    float *grad_matmul_out2 = malloc(sizeof(matmul_out2));
    add_time_embedding_backward(grad_conv_out_concat, grad_conv_outbr3, grad_matmul_out2, BATCH_SIZE, 64, 7, 7);
    
    // time embeddings
    bias_backward(nn->grad_bias_m2, grad_matmul_out2, BATCH_SIZE, TIME_HIDDEN_SIZE);
    float *grad_matmul_outs1 = malloc(sizeof(matmul_outs1));
    matmul_a_bt(grad_matmul_out2, nn->matmul_2, grad_matmul_outs1, BATCH_SIZE, TIME_HIDDEN_SIZE, TIME_HIDDEN_SIZE);
    matmul_at_b(matmul_outs1, grad_matmul_out2, nn->grad_matmul_2);
    float *grad_matmul_out1 = malloc(sizeof(matmul_out1));
    sin_backward(matmul_out1, grad_matmul_outs1, grad_matmul_out1);
    float *grad_time = malloc(sizeof(time));
    matmul_a_bt(grad_matmul_out1, nn->matmul_1, grad_time, BATCH_SIZE, 1, TIME_HIDDEN_SIZE);
    matmul_at_b(time, grad_matmul_out1, nn->grad_matmul_1);

    // bottleneck
    float *grad_conv_out3 = malloc(sizeof(conv_out3));
    float *grad_conv_outbr3 = malloc(sizeof(conv_outbr3));
    relu_backward(conv_out3, grad_conv_outbr3, grad_conv_out3, 64, 7, 7);
    float *grad_mean_3 = malloc(sizeof(mean_3));
    float *grad_std_3 = malloc(sizeof(std_3));
    groupnorm_backward(conv_out3, mean_3, std_3, grad_conv_out3, grad_conv_out3, grad_mean_3, grad_std_3, ,64, 8, 7, 7);
    float *grad_conv_outbr_max2 = malloc(sizeof(conv_outbr_max2));
    conv2d_backward(conv_outbr_max2, conv_out3, nn->filter_3, grad_conv_outbr_max2, grad_conv_out3, nn->grad_filter_3, 1, 32, 64, 7, 7);

    // enc block2    
    float *grad_conv_outbr2 = malloc(sizeof(conv_outbr2));
    float *grad_conv_out2 = malloc(sizeof(conv_out2));
    ...
    float *grad_mean_2 = malloc(sizeof(mean_2));
    float *grad_std_2 = malloc(sizeof(std_2));
    groupnorm_backward(conv_out2, mean_2, std_2, grad_conv_out2, grad_conv_out2, grad_mean_2, grad_std_2, 32, 4, 14, 14);
    bias_conv_backward(grad_conv_out2, nn->grad_bias2, 32, 14, 14);
    float *grad_conv_outbr_max1 = malloc(sizeof(conv_outbr_max1));
    conv2d_backward(conv_outbr_max1, conv_out2, nn->filter_2, grad_conv_outbr_max1, grad_conv_out2, nn->grad_filter_2, 1, 16, 32, 14, 14);

    // enc block1
    float *grad_conv_outbr1 = malloc(sizeof(conv_outbr1));
    float *grad_conv_out1 = malloc(sizeof(conv_out1));
    maxpool2d_backward(conv_outbr1, conv_outbr_max1, grad_conv_outbr1, grad_conv_outbr_max1, 16, 28, 28, 14, 14);
    relu_backward(conv_out1, grad_conv_outbr1, grad_conv_out1, 16, 28, 28);
    float *grad_mean_1 = malloc(sizeof(mean_1));
    float *grad_std_1 = malloc(sizeof(std_1));
    groupnorm_backward(conv_out1, mean_1, std_1, grad_conv_out1, grad_conv_out1, grad_mean_1, grad_std_1, 16, 4, 28, 28);
    bias_conv_backward(grad_conv_out1, nn->grad_bias1, 16, 28, 28);
    float *grad_inputs = malloc(sizeof(grad_inputs));
    conv2d_backward(inputs, conv_out1, nn->filter_1, grad_inputs, grad_conv_out1, nn->grad_filter_1, 1, 1, 16, 28, 28);

    // float *grad_input_add = malloc(sizeof(input)); 
    // float *grad_noise_add = malloc(sizeof(noise));
    // noise_addition_backward(grad_output_final, grad_input_add, grad_noise_add, timesteps, sqrt_alpha_bar, sqrt_one_minus_alpha_bar, BATCH_SIZE, 1, HEIGHT, WIDTH);
...
void train_timed(Network *nn, float *X_train){    
    ...
    float *conv_outbr_max2 = malloc(BATCH_SIZE * CHANNEL_2 * HEIGHT/2 * WIDTH/2 *  sizeof(float));
    ...
    // time embeddings
    float *matmul_out1 = malloc(TIME_HIDDEN_SIZE * sizeof(float));
    float *matmul_outs1 = malloc(TIME_HIDDEN_SIZE * TIME_HIDDEN_SIZE * sizeof(float));
    float *matmul_out2 = malloc(TIME_HIDDEN_SIZE * sizeof(float));
    ...
    float *output = malloc(BATCH_SIZE * HEIGHT * WIDTH * sizeof(float));
    ...
    float *noise_added_input = malloc(BATCH_SIZE * HEIGHT * WIDTH);
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++){
        ...
        for (int batch = 0; batch < num_batches; batch++) {
            ...
            memset(noise_added_input, 0, BATCH_SIZE * HEIGHT * WIDTH * sizeof(float));

            noise_addition(batch_input, noise_added_input, eps, sqrtab, sqrtmab, BATCH_SIZE, 1, HEIGHT, WIDTH);

            forward_timed(nn, batch_input, output, conv_out1, conv_outbr1, conv_outbr_max1, 
                conv_out2, conv_outbr2, conv_outbr_max2, conv_out3, conv_outbr3, conv_out4, 
                conv_outbc4, conv_outbr4, conv_out5, conv_outbc5, conv_outbr5, 
                mean_1, std_1, mean_2, std_2, mean_3, std_3, mean_4, std_4, mean_5, std_5);
            ...
            float loss = mse_loss(output, eps, BATCH_SIZE);
            total_loss += loss;
...
void initialize_random_weights(NeuralNetwork *nn) {
    ...
    initialize_bias(nn->bias1, CHANNEL_1);
    initialize_bias(nn->bias2, CHANNEL_2);
    initialize_bias(nn->bias3, CHANNEL_3);
    initialize_bias(nn->bias4, CHANNEL_2);
    initialize_bias(nn->bias41, CHANNEL_2);
    initialize_bias(nn->bias5, CHANNEL_1);
    initialize_bias(nn->bias51, CHANNEL_1);
    initialize_bias(nn->bias6, 1);
    ...
    initialize_matmul_weights(nn->bias_m2, TIME_HIDDEN_SIZE);
}

void initialize_neural_network(Network *nn){
    nn->filter_1 = malloc(CHANNEL_1 * 3 * 3 * sizeof(float));
    ...
    nn->filter_6 = malloc(CHANNEL_1 * sizeof(float));
    nn->bias_6 = malloc(1 * sizeof(float));

    nn->matmul_1 = malloc(TIME_HIDDEN_SIZE * sizeof(float));
